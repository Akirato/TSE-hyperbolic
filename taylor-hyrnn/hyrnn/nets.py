...
import math
from ts_poincare import TSPoincareBall
import geoopt

pmath = TSPoincareBall()
def mobius_linear(
    input,
    weight,
    bias=None,
    ...
    if nonlin is not None:
        output = pmath.expmap0(nonlin(pmath.logmap0(output,c=c)),c=c)
...
def mobius_gru_cell(
    ...
    if nonlin is not None:
        ...
        h_tilde = pmath.expmap0(nonlin(pmath.logmap0(h_tilde,c=c)),c=c)
    delta_h = pmath.mobius_add(-hx, h_tilde, c=c)
    h_out = pmath.mobius_add(hx, z_t*delta_h, c=c)
    return h_out
