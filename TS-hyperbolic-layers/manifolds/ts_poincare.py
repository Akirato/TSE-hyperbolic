...
#from utils.ts_math_utils import artanh, tanh
...
class TSPoincareBall(Manifold):
    ...
    def __init__(self, ):
        super(TSPoincareBall, self).__init__()
        self.eps = {torch.float32: 4e-3, torch.float64: 1e-5}

    @classmethod
    def sqdist(self, p1, p2, c):
        sqrt_c = c ** 0.5
        val = sqrt_c * self.mobius_add(-p1, p2, c, dim=-1).norm(dim=-1, p=2, keepdim=False).clamp(min=self.min_norm, max=self.max_norm)
        dist = artanh(val) * 2 / sqrt_c
        return (dist ** 2).clamp(min=-self.euclidean_max, max=self.euclidean_max)
    ...
    def proj(self, x, c):
        ...
        """
        norm = torch.clamp_min(x.norm(dim=-1, keepdim=True, p=2), TSPoincareBall.min_norm)
        maxnorm = (1 - TSPoincareBall.eps[x.dtype]) / (c ** 0.5)
        cond = norm > maxnorm
    ...
    def expmap(self, u, p, c):
        sqrt_c = c ** 0.5
        u_norm = u.norm(dim=-1, p=2, keepdim=True).clamp_min(TSPoincareBall.min_norm)
        second_term = (
                tanh(sqrt_c / 2 * TSPoincareBall._lambda_x(p, c) * u_norm)
                * u
                / (sqrt_c * u_norm)
        )
        gamma_1 = TSPoincareBall.mobius_add(p, second_term, c)
        return gamma_1

    @classmethod
    def logmap(self, p1, p2, c):
        sub = TSPoincareBall.mobius_add(-p1, p2, c)
        sub_norm = sub.norm(dim=-1, p=2, keepdim=True).clamp_min(TSPoincareBall.min_norm)
        lam = TSPoincareBall._lambda_x(p1, c)
        sqrt_c = c ** 0.5
    ...
    def expmap0(self, u, c, dim: int = -1):
        ...
        """
        sqrt_c = c ** 0.5
        u_norm = torch.clamp_min(u.norm(dim=dim, p=2, keepdim=True), TSPoincareBall.min_norm)
    ...
    def logmap0(self, p, c, dim: int = -1):
        ...
        """
        sqrt_c = c ** 0.5
        p_norm = p.norm(dim=dim, p=2, keepdim=True).clamp_min(TSPoincareBall.min_norm)
        y = (sqrt_c * p_norm).clamp(min=TSPoincareBall.min_norm, max=TSPoincareBall.max_norm)
        return p / (y * artanh(y))
    ...
    def mobius_add(self, x, y, c=1, dim=-1):
        ...
        denom = (1 + 2 * c * xy + c ** 2 * x2 * y2).clamp_min(TSPoincareBall.min_norm)
        return (num / denom).clamp(min=-TSPoincareBall.euclidean_max, max=TSPoincareBall.euclidean_max)
    ...
    def mobius_mul(self, x, t, dim=-1):
        ...
        normx = x.norm(dim=dim, p=2, keepdim=True).clamp(min=TSPoincareBall.min_norm, max=TSPoincareBall.max_norm)
        t = t.unsqueeze(dim)
    ...
    def mobius_matvec(self, m, x, c, dim: int = -1):
        ...
        x_norm = x.norm(dim=dim, keepdim=True, p=2).clamp_min(TSPoincareBall.min_norm)
        sqrt_c = c ** 0.5 if c else TSPoincareBall.min_norm
        if dim != -1 or m.dim() == 2:
            mx = torch.tensordot(x, m, dims=([dim], [1]))
        else:
            mx = torch.matmul(m, x.unsqueeze(-1)).squeeze(-1)
        mx_norm = mx.norm(dim=dim, keepdim=True, p=2).clamp(min=TSPoincareBall.min_norm, max=TSPoincareBall.max_norm)

        res_c = tanh(mx_norm / x_norm * artanh((sqrt_c * x_norm).clamp(min=TSPoincareBall.min_norm, max=TSPoincareBall.max_norm))) * mx / (mx_norm * sqrt_c)
    ...
    def mobius_bmm(self, bm, bx, c = 1):
        ...
        for i in range(0, bm.shape[0]):
            batch_m.append(TSPoincareBall.mobius_matvec(bm[i], bx[i], c))
    ...
    def _gyration(self, u, v, w, c, dim: int = -1):
        ...
        b = -c2 * vw * u2 - c * uw
        d = 1 + 2 * c * uv + c2 * u2 * v2
        return w + 2 * (a * u + b * v) / d.clamp_min(TSPoincareBall.min_norm)

    @classmethod
    def inner(self, x, c, u, v=None, keepdim=False):
        if v is None:
            v = u
        lambda_x = TSPoincareBall._lambda_x(x, c)
        return lambda_x ** 2 * (u * v).sum(dim=-1, keepdim=keepdim)

    @classmethod
    def ptransp(self, x, y, u, c):
        lambda_x = TSPoincareBall._lambda_x(x, c)
        lambda_y = TSPoincareBall._lambda_x(y, c)
        return TSPoincareBall._gyration(y, -x, u, c) * lambda_x / lambda_y
    ...
    def ptransp0(self, x, u, c):
        lambda_x = TSPoincareBall._lambda_x(x, c)
        return 2 * u / lambda_x.clamp_min(TSPoincareBall.min_norm)
    ...
    def from_hyperboloid(self, x, c=1, ideal=False):
        ...
        """
        if ideal:
            return x[..., 1:] / (x[..., 0].unsqueeze(-1)).clamp_min(TSPoincareBall.min_norm)
        else:
            sqrtK = (1. / c) ** 0.5
            return sqrtK * x[..., 1:] / (sqrtK + x[..., 0].unsqueeze(-1)).clamp_min(TSPoincareBall.min_norm)
    ...
    def distance(self, x, y, keepdim=True):
        ...
        """
        pairwise_norm = TSPoincareBall.mobius_add(-x, y).norm(dim=-1, p=2, keepdim=True)
        dist = 2.0 * torch.atanh(pairwise_norm.clamp(-1 + TSPoincareBall.min_norm, 1 - TSPoincareBall.max_norm))
        if not keepdim:
            dist = dist.squeeze(-1)
        return dist
    ...
    def pairwise_distance(self, x, keepdim=False):
        ...
        return TSPoincareBall.distance(x.unsqueeze(-2), x.unsqueeze(-3), keepdim=keepdim)
    ...
    def mobius_midpoint_pair(self, x, y):
        ...
        t1 = TSPoincareBall.mobius_add(-x, y)
        t2 = TSPoincareBall.mobius_mul(t1, 0.5)
        return TSPoincareBall.mobius_add(x, t2)
    ...
    def _sq_gamma(self, v, r = 1):
        ...
        returns:
            ...
            in r-ball.
        """
        return 1/(1 - torch.pow(TSPoincareBall.distance0(v)/r, 2)).clamp(min=TSPoincareBall.min_norm, max=TSPoincareBall.max_norm)
    ...
    def mobius_midpoint(self, v, a=None):
        ...
        if a is not None:
            ...
            w_v = TSPoincareBall.mobius_mul(v, torch.nn.functional.normalize(a, dim=-1))
        else:
            w_v = v
        # Calculates the gamma factors for all vectors        
        gamma_ws = torch.Tensor([TSPoincareBall._sq_gamma(w_v_j) for _, w_v_j in enumerate(w_v)])
        weights = (gamma_ws / (torch.sum(gamma_ws) - w_v.shape[0] / 2)).reshape(w_v.shape[0], 1)
        out = TSPoincareBall.mobius_mul(x=torch.sum(weights * v, dim=0), t=torch.tensor(0.5))
    ...
    def poincare_attention_weight(self, q, k, beta, c):
        ...
        return torch.stack([TSPoincareBall.expmap(beta*TSPoincareBall.distance(q[idx], k[idx]) - c) for idx in enumerate(q)])
    ...
    def poincare_aggregation(self, q, k, v):
        ...
        for i, v_i in enumerate(v):
            h.append([TSPoincareBall._mobius_midpoint(TSPoincareBall.poincare_attention_weight(q[i], k[i]), v_i)])
    ...
    def euclidean2poincare(self, x, c, scale = 1):
        ...
        """
        p_c = c[0] * TSPoincareBall.max_norm
        norm = torch.norm(x, dim=-1).clamp(min=TSPoincareBall.min_norm, max=TSPoincareBall.euclidean_max) / scale
        x_exp = torch.exp(-norm).clamp(min=TSPoincareBall.min_norm, max=TSPoincareBall.euclidean_max)
    ...
    def poincare2euclidean(self, p, c, scale = 1):
        ...
        lg = torch.log(((c - norm)/(c + norm)).clamp_min(TSPoincareBall.min_norm)).clamp(min=-TSPoincareBall.euclidean_max, max=TSPoincareBall.euclidean_max)
        coef = (-lg / norm.clamp_min(TSPoincareBall.min_norm)).clamp(min=-TSPoincareBall.euclidean_max, max=TSPoincareBall.euclidean_max)
        return scale * (coef.unsqueeze(-1) *  p).clamp(min=-TSPoincareBall.euclidean_max, max=TSPoincareBall.euclidean_max)
